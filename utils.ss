(library (utils)
  (export vector-splice
          ...
          save
          restore
          parse-cmdline-number
          parse-cmdline)

  (import
   (chezscheme))

  (define (make-cmdline-parser f)
    (define (parse-cmdline . args)
      (define (check-param params args)
        (cond
         ((member (car params) args)
          (cadr params))
         (else #f)))
      (let iterate ((params (command-line)))
        (cond
         ((null? params) #f)
         ((check-param params args) => f)
         (else (iterate (cdr params))))))
    parse-cmdline)

  (define parse-cmdline-number (make-cmdline-parser string->number))
  (define parse-cmdline (make-cmdline-parser (lambda (s) s)))
