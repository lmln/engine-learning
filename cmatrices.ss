(library (cmatrices)
  ...
  (define-syntax define-guarded
    (syntax-rules ()
      ((define-guarded (name arg ...) e1 e2 ...)
       (define (name arg ...)
         (let ((result e1))
           (cmatrix-guardian result)
           e2 ...
           result)))))

  (define-guarded (make-matrix rows columns value)
    (make-cmatrix rows columns value)
  ...
  (define-guarded (make-random-matrix rows columns)
    (make-random-xavier-cmatrix rows columns))

  (define-guarded (matrix-hadamard m n)
    (chadamard m n))

  (define-guarded (matrix-sum m n)
    (csum m n))

  (define-guarded (matrix-dif m n)
    (cdif m n))

  (define-guarded (matrix-mul m n)
    (cmul m n)
    ;;(set! matrix-count (+ matrix-count 1))
    )

  (define-guarded (matrix-scale m n)
    (cscale m n))

  (define-guarded (matrix-sigmoid m)
    (csigmoid m))

  (define-guarded (matrix-sigmoid-derivative m)
    (csigmoid-derivative m))

  (define-guarded (matrix-transpose m)
    (ctranspose m))
